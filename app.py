...
def diagnosticar_grade(turmas, professores, disciplinas, aulas_alocadas):
    ...
    if aulas_alocadas and hasattr(aulas_alocadas[0], 'turma'):
        aulas = [a.__dict__ for a in aulas_alocadas]
    else:
        aulas = aulas_alocadas
    
    # 1. ANÁLISE POR TURMA
    total_aulas_necessarias = 0
    total_aulas_alocadas = len(aulas)
    ...
    for professor in professores:
        ...
        aulas_professor = len([a for a in aulas if a['professor'] == professor.nome])
...
class CompletadorDeGrade:
    ...
    def completar_grade(self, aulas_atuais):
        ...
        # Converter para dicionários se necessário
        if aulas_atuais and hasattr(aulas_atuais[0], 'turma'):
            aulas = [a.__dict__ for a in aulas_atuais]
        else:
    ...
    def _pode_alocar(self, aulas, turma, professor, dia, horario):
        ...
        for aula in aulas:
            if aula['turma'] == turma and aula['dia'] == dia and aula['horario'] == horario:
                return False
        
        # Verificar se professor já tem aula neste horário
        for aula in aulas:
            if aula['professor'] == professor and aula['dia'] == dia and aula['horario'] == horario:
                return False
...
def salvar_grade_como(nome, aulas, config):
    ...
    # Converter para dicionários se for objetos
    if aulas and hasattr(aulas[0], 'turma'):
        aulas_dict = [a.__dict__ for a in aulas]
    else:
        aulas_dict = aulas
                ...
                with st.spinner(f"Gerando grade para {grupo_texto}..."):
                    try:
                        ...
                        if tipo_grade == "Grade por Turma Específica" and turma_selecionada:
                            # Convertendo para dicionários se necessário
                            if aulas and hasattr(aulas[0], 'turma'):
                                aulas = [a for a in aulas if a.turma == turma_selecionada]
                            else:
                                aulas = [a for a in aulas if a['turma'] == turma_selecionada]
                        
                        # 🔧 CORREÇÃO CRÍTICA: Padronizar todas as aulas como dicionários
                        if aulas and hasattr(aulas[0], 'turma'):
                            st.session_state.aulas = [a.__dict__ for a in aulas]
                        else:
                            st.session_state.aulas = aulas
                        
                        if salvar_tudo():
                            st.success(f"✅ Grade {grupo_texto} gerada com {metodo}! ({len(st.session_state.aulas)} aulas)")
                        ...
                        if st.session_state.aulas:
                            ...
                            diagnostico = diagnosticar_grade(turmas_filtradas, professores_filtrados, disciplinas_filtradas, st.session_state.aulas)
                            ...
                            if diagnostico['completude'] < 100:
                                ...
                                with col1:
                                    if st.button("🔧 TENTAR COMPLETAR GRADE", type="primary", use_container_width=True):
                                        with st.spinner("Tentando completar a grade..."):
                                            ...
                                            aulas_completas = completador.completar_grade(st.session_state.aulas)
                                ...
                                with col2:
                                    ...
                                    if st.button("💾 SALVAR GRADE", type="secondary", use_container_width=True):
                                        if salvar_grade_como(nome_grade, st.session_state.aulas, {'tipo': tipo_grade, 'algoritmo': metodo}):
                        ...
                        if st.session_state.aulas:
                            ...
                            # PADRÃO: 'aulas' já são dicionários após a correção
                                ...
                                for periodo in periodos:
                                    ...
                                    for dia in dias_ordenados:
                                        ...
                                        for aula in aulas_turma:
                                            if aula['dia'] == dia and aula['horario'] == periodo:
                                                aula_encontrada = aula
                                                break
                                        
                                        if aula_encontrada:
                                            # Obter dados da aula
                                            disciplina = str(aula_encontrada['disciplina']).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                                            professor = str(aula_encontrada['professor']).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                            ...
                            df_aulas = pd.DataFrame([
                                {
                                    ...
                                    "Segmento": a.get('segmento', obter_segmento_turma(a['turma']))
                                }
...
with abas[6]:
    ...
    if not st.session_state.get('aulas'):
        ...
        col1, col2 = st.columns(2)
        with col1:
            # CORREÇÃO: Usar a sintaxe de dicionário corretamente
        ...
        if professor_selecionado:
            # Filtrar aulas do professor (sempre dicionários)
